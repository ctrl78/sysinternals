...
node['sysinternals']['install_dir_version'] = "#{node['sysinternals']['install_dir_full']}-#{version}"

directory node['sysinternals']['install_dir_version'] do
  action :create
end

windows_zipfile node['sysinternals']['install_dir_version'] do
  action :unzip
  ...
  notofies :create, "link[@node['sysinternals']['install_dir_version'].to_s]", :immediately
end
